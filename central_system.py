...
stops = ['Centraal Station', 'Station Heyendaal-1', 'Huygensgebouw',
         'Erasmus', 'Spinozegebouw', 'Radboud/UMC', 'HAN', 'Station Heyendaal-2']
...
class Central_system:
    ...
    def get_to_stop(self):
        print("I am printing ", self.to_stop_pods)
        return self.to_stop_pods
                ...
                with conn:
                    ...
                    if res[:7] == 'ASK_ID ':
                        ...
                        if serial_number == -1:
                            ...
                            conn.sendall(
                                'ERR_SERIAL_NUMBER_NOT_FOUND'.encode('UTF-8'))
                        else:
                        ...
                        if (station_id not in stop_ids):
                            ...
                            closest_pod = 0
                            distance = len(stops)
                            for (x, y) in self.current_loc.items():
                                # checks how far the pod is, here we have +1 because the location is last location
                                if(((stop_ids.index(station_id) - (stop_ids.index(y)+1)) % len(stops)) < distance):
                                    distance = (stop_ids.index(
                                        station_id) - (stop_ids.index(y)+1)) % len(stops)
                                    closest_pod = x
                            if got_off == 0:  # check if the pod arrived or is requested
                                self.to_stop_pods[closest_pod][stop_ids.index(
                                    station_id)] = 1
                            else:
                                self.to_stop_pods[closest_pod][stop_ids.index(
                                    station_id)] = 0
